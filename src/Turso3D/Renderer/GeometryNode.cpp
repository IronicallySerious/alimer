...
namespace Turso3D
{
    ...
    void GeometryNode::SetGeometry(size_t index, Geometry* geometry)
    {
        if (!geometry)
        {
            TURSO3D_LOGERROR("Can not assign null geometry");
            return;
        }
        ...
        if (index < batches.Size())
            ...
            TURSO3D_LOGERRORF("Out of bounds batch index %d for setting geometry", (int)index);
    }
    ...
    void GeometryNode::SetMaterial(size_t index, Material* material)
    {
        if (index < batches.Size())
        {
            ...
            TURSO3D_LOGERRORF("Out of bounds batch index %d for setting material", (int)index);
    }
