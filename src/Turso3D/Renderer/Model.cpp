...
namespace Turso3D
{
    ...
    bool Model::BeginLoad(Stream& source)
    {
        ...
        {
            TURSO3D_LOGERROR(source.Name() + " is not a valid model file");
            return false;
        }
        ...
        for (size_t i = 0; i < numVertexBuffers; ++i)
        {
            ...
            vbDesc.vertexCount = source.Read<unsigned>();
            ...
            vbDesc.vertexData = new uint8_t[vbDesc.vertexCount * vertexSize];
            source.Read(&vbDesc.vertexData[0], vbDesc.vertexCount * vertexSize);
        }
        ...
        for (size_t i = 0; i < numIndexBuffers; ++i)
        {
            ...
            ibDesc.indexCount = source.Read<unsigned>();
            const uint32_t indexSize = source.Read<unsigned>();
            ibDesc.indexType = indexSize == 4u ? IndexType::UInt32 : IndexType::UInt16;
        ...
        if (numMorphs)
        {
            TURSO3D_LOGERROR("Models with vertex morphs are not supported for now");
            return false;
        }
    ...
    bool Model::EndLoad()
    {
        ...
        for (size_t i = 0; i < ibDescs.Size(); ++i)
        {
            ...
            ib->Define(ResourceUsage::Immutable, ibDesc.indexCount, ibDesc.indexType, true, ibDesc.indexData.Get());
            ibs.Push(ib);
        }
        ...
        for (size_t i = 0; i < geomDescs.Size(); ++i)
        {
            ...
            for (size_t j = 0; j < geomDescs[i].Size(); ++j)
            {
                ...
                if (geomDesc.vbRef < vbs.Size())
                    ...
                    TURSO3D_LOGERROR("Out of range vertex buffer reference in " + Name());

                if (geomDesc.ibRef < ibs.Size())
                    geom->indexBuffer = ibs[geomDesc.ibRef];
                else
                    TURSO3D_LOGERROR("Out of range index buffer reference in " + Name());
    ...
    void Model::SetNumLodLevels(size_t index, size_t num)
    {
        ...
        {
            TURSO3D_LOGERROR("Out of bounds geometry index for setting number of LOD levels");
            return;
        }
